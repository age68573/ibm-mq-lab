...
const performGetQMAddress = async () => {
  try {
    ...
    const response = await fetch(`http://10.107.72.22:8080/address`);
...
const performSend = async (msg) => {
  try {
    const response = await fetch(
      `http://10.107.72.22:8080/send?msg=${encodeURIComponent(msg)}`
    );
...
const performGet = async () => {
  try {
    const response = await fetch(`http://10.107.72.22:8080/recv`);
